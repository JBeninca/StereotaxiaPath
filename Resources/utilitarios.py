# -*- coding: utf-8 -*-
# utilitarios de StereotaxiaPath version 22.0809
...
def Inicializa_Escena(modulo):
    slicer.mrmlScene.Clear(0)       
    param = Genera_Nodo("vtkMRMLScriptedModuleNode", "Param_data")
...
def Genera_Nodo(clase, nombre_Nodo):
    borra_nodo_si_existe(nombre_Nodo)  # if exists
...
def genera_Markup(nombre):    
    borra_nodo_si_existe(nombre)  # if exists
...
def borra_nodo_si_existe(nombre):
    try:
...
def Obtiene_Centro_de_Masa(nodo_volu, fidu_nume, fidu_RAS, roi_size, umbral):
...
def calcula_angulos_con_nodo(Target, Entry):
    #import math
    x = Entry[0]-Target[0]
    y = Entry[1]-Target[1]
    z = Entry[2]-Target[2]
...
def calcula_angulos_con_vtk(Target, Entry):
...
def calcula_angulos_simple(Target, Entry):
...
def ubicar_la_punta_en_RPN(p_RAS):
    #print("punta =", p_RAS)
